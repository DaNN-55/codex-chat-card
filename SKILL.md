...
所有命令都通过 `node scripts/run.mjs` 执行。首次运行缺少依赖时，入口会使用锁定版本自动安装运行依赖并继续原命令；如果安装被网络或目录权限阻止，请取得对应授权后重试同一命令一次，不要要求用户运行构建。无法定位当前会话时，请用户提供明确的 Codex JSONL 路径，不要自行选择其他会话。
