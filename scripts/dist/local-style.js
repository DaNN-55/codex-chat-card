...
function mix(first, second, secondWeight, fallback) {
    const a = hexToRgb(first);
    const b = hexToRgb(second);
    if (!a || !b)
        return fallback;
    const channel = (index) => Math.round(a[index] * (1 - secondWeight) + b[index] * secondWeight);
    return `#${[0, 1, 2].map((index) => channel(index).toString(16).padStart(2, "0")).join("")}`;
}
async function detectSystemAppearance() {
...
function supportedUiFont(value) {
    const configured = typeof value === "string" ? value : "";
    return /\b(Geist|Inter)\b/i.test(configured)
        ? "Geist, Noto Sans SC"
        : "Noto Sans SC";
}
export async function resolveLocalCodexTheme(options = {}) {
    ...
    try {
        ...
        const preference = stringValue(desktop.appearanceTheme, "system").toLowerCase();
        const appearance = preference === "dark" || preference === "light"
            ? preference
            : (options.systemAppearance ?? (await detectSystemAppearance()));
        const chrome = record(desktop[appearance === "dark"
            ? "appearanceDarkChromeTheme"
            : "appearanceLightChromeTheme"]);
        const fonts = record(chrome.fonts);
        const surface = stringValue(chrome.surface, appearance === "dark" ? "#111111" : "#ffffff");
        const ink = stringValue(chrome.ink, appearance === "dark" ? "#fcfcfc" : "#0d0d0d");
        const chromeAccent = stringValue(chrome.accent, appearance === "dark" ? "#6ea8e8" : "#3a83f7");
...
export async function resolveExportTheme(requested) {
