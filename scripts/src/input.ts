...
import type { ConversationDocument, ConversationRound } from "./types.js";
...
export function sanitizeUserMessage(text: string): string {
  const hasAttachmentEnvelope = /^\s*# Files mentioned by the user:/m.test(
    text,
  );
  if (!hasAttachmentEnvelope) return text.trim();
...
function currentDesktopRounds(records: JsonRecord[]): ConversationRound[] {
  const byTurn = new Map<string, PartialRound>();
  const order: string[] = [];

  const getRound = (turnId: string, timestamp?: string) => {
...
function legacyRounds(records: JsonRecord[]): ConversationRound[] {
  ...
  for (const record of records) {
    ...
    if (
      ...
      continue;
    if (!pending || payload.phase === "commentary") continue;
...
export async function loadConversation(
  ...
  if (rounds.length === 0) {
    throw new Error(
      "No completed Codex conversation rounds were found in this session.",
    );
  }
  return { sessionId, sourcePath: inputPath, rounds };
}

async function findByThreadId(
  directory: string,
  threadId: string,
): Promise<string | null> {
  let entries;
...
export async function resolveCurrentSession(): Promise<string> {
  const threadId = process.env.CODEX_THREAD_ID;
  if (!threadId)
    throw new Error(
      "CODEX_THREAD_ID is not set; pass --input with a Codex JSONL path.",
    );
