...
function exportMode(value?: string): ExportMode {
...
function mockupMode(value?: string): MockupMode {
...
function contentView(value?: string): ContentView {
...
async function inputPath(
  ...
  if (flags.has("current")) return resolveCurrentSession();
...
async function list(flags: Map<string, string | boolean>) {
...
async function exportImage(flags: Map<string, string | boolean>) {
  const document = await loadConversation(await inputPath(flags));
  ...
  const format = (
    stringFlag(flags, "format") ??
    (outputFlag ? extname(outputFlag).slice(1) : "png")
  ).toLowerCase();
  ...
  else await writeFile(output, await renderToPng(options));
  const result = {
    output,
    format,
    theme: theme.name,
...
async function main() {
  ...
  else if (command === "export") await exportImage(flags);
