...
export type EmojiAssets = Map<string, string>;
...
export async function loadFonts() {
  const [
    ...
    Object.values(FONT_FILES).map((file) =>
      readFile(fileURLToPath(import.meta.resolve(file))),
    ),
  );
  return [
    {
...
export async function loadEmojiAssets(texts: string[]): Promise<EmojiAssets> {
  const found = new Set<string>();
  for (const text of texts) {
    const regex = emojiRegex();
    for (const match of text.matchAll(regex)) found.add(match[0]);
  }
